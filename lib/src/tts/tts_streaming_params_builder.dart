...
class TtsStreamingParamsBuilder extends TtsParamsBuilder {
  ...
  TtsStreamingParamsBuilder maxLatency(Duration latency) {
    ...
    if (latency.inMilliseconds < 50) {
      throw ArgumentError('Max latency cannot be less than 50ms (current: ${latency.inMilliseconds}ms)');
    }
    _maxLatency = latency;
    return this;
  }
  ...
  TtsStreamingParamsBuilder copy() {
    final newBuilder = TtsStreamingParamsBuilder();
    
    // Copy base parameters
    final baseBuilder = super.copy();
    newBuilder._voice = baseBuilder._voice;
    newBuilder._text = baseBuilder._text;
    newBuilder._audioFormat = baseBuilder._audioFormat;
    newBuilder._rate = baseBuilder._rate;
    newBuilder._style = baseBuilder._style;
    newBuilder._role = baseBuilder._role;
